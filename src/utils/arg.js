// Lifted from https://github.com/vercel/arg/blob/main/index.js (MIT license)
// Note: Line 151 has been modified to support both boolean and string arguments with the same name
"use strict"
  ...
  for (let i = 0, len = argv.length; i < len; i++) {
    ...
    if (wholeArg.length > 1 && wholeArg[0] === "-") {
      ...
      for (let j = 0; j < separatedArguments.length; j++) {
        ...
        if (isFlag) {
          result[argName] = type(
            argStr ? argStr : true, // NOTE: CHANGED! Otherwise --allow-net="foo" & --allow-net at the same time wouldn't be possible
            argName,
            result[argName],
          )
